...
class Container:
    ...
    def initialize(self, config_path: str) -> None:
        ...
        # NEW: Protections (circuit breakers)
        self._protections = create_default_protections()
        logger.info("Protections initialized (StoplossGuard, MaxDrawdown, CooldownPeriod)")
    ...
    def set_broker_connector(self, connector) -> None:
        ...
        if self._config:
            self._user_stream = UserStreamTracker(
                ...
                is_paper=(self._config.account.mode == 'paper')
            )
            ...
            logger.info(f"User stream tracker initialized (paper={self._config.account.mode == 'paper'})")
