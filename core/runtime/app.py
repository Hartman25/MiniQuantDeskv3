...
def run(opts: RunOptions) -> int:
    ...
    run_id = TradeJournal.new_run_id()
    ...
    try:
        ...
        # Wire canonical trade journal into execution engine (optional injection)
        if exec_engine:
            exec_engine.set_trade_journal(trade_journal, run_id=run_id)
        ...
        # Optional: dynamic universe (CORE + accepted) via data/universe/universe_active.json
        ...
        # (A) strategies: [ {name, enabled, ...}, ... ]
        # (B) strategies: { enabled: [ ... ] }
        ...
        for strat_cfg in enabled_strategies:
            # Normalize dict vs object config
        ...
        # Map symbol -> broker_order_id for protective stops (kept in-memory for now)
        ...
        while state.running:
            try:
                ...
                for symbol in all_symbols:
                    ...
                    for sig in signals:
                        # PATCH 5: journal the raw signal with stable trade_id
                        trade_id = sig.get("trade_id") or f"{sig.get('strategy','UNKNOWN')}:{sig.get('symbol', symbol)}:{datetime.now(timezone.utc).date().isoformat()}:{uuid.uuid4().hex[:10]}"
                        ...
                        if prot_result.is_protected:
                            journal.write_event({"event": "protection_block", "trade_id": trade_id, "symbol": sig_symbol, "side": side_str, "qty": str(qty), "reason": prot_result.reason})
                            logger.warning(
                        ...
                        journal.write_event({"event": "risk_decision", "trade_id": trade_id, "approved": bool(getattr(risk, "approved", False)), "reason": getattr(risk, "reason", None), "details": (risk.to_dict() if hasattr(risk, "to_dict") else {})})
                        ...
                        # -------------------------------------------------------------------
                        # PATCH 3: Risk enforcement (sizing + position sanity)
                        # -------------------------------------------------------------------
                        try:
                            if broker_side == BrokerOrderSide.BUY:
                                for _attr in ("approved_qty", "approved_quantity", "capped_qty", "capped_quantity", "sized_qty", "sized_quantity"):
                            ...
                            if hasattr(broker, "get_position"):
                                ...
                                if pos is not None:
                                    ...
                                    if broker_side == BrokerOrderSide.BUY and pos_qty > 0:
                                        logger.info("RISK_SKIP_ALREADY_IN_POSITION", extra={"symbol": sig_symbol, "pos_qty": str(pos_qty), "requested_qty": str(qty)})
                                        continue
                                    ...
                                    if broker_side == BrokerOrderSide.SELL:
                                        if pos_qty <= 0:
                                            ...
                                            continue
                                        if qty > pos_qty:
                                            logger.info("RISK_CAP_SELL_QTY_TO_POSITION", extra={"symbol": sig_symbol, "requested_qty": str(qty), "pos_qty": str(pos_qty)})
                                            qty = pos_qty
                        ...
                        if broker_side == BrokerOrderSide.SELL:
                            ...
                            if stop_id:
                                journal.write_event({"event": "protective_stop_cancel_requested", "trade_id": trade_id, "symbol": sig_symbol, "stop_broker_order_id": stop_id, "reason": "exit_signal_cancel_protective_stop"})
                                try:
                        ...
                        if order_type == "LIMIT":
                            ...
                            journal.write_event({"event": "order_submitted", "trade_id": trade_id, "internal_order_id": internal_id, "broker_order_id": broker_order_id, "symbol": sig_symbol, "side": side_str, "qty": str(qty), "order_type": "LIMIT", "limit_price": str(limit_price_d), "strategy": sig_strategy})
                            ...
                            if final_status in (OrderStatus.CANCELLED, OrderStatus.REJECTED):
                                try:
                                    exec_engine.cancel_order(
                                        internal_order_id=internal_id,
                                        broker_order_id=broker_order_id,
                                        reason="limit_entry_terminal",
                                    )
                                except Exception:
                                    pass
                                journal.write_event({"event": "order_cancelled", "trade_id": trade_id, "internal_order_id": internal_id, "broker_order_id": broker_order_id, "reason": "limit_entry_terminal", "final_status": getattr(final_status, "value", str(final_status))})
                                logger.info("LIMIT_ENTRY_NOT_FILLED_TERMINAL", extra={"symbol": sig_symbol, "strategy": sig_strategy, "final_status": final_status.value})
                                continue

                            if final_status not in (OrderStatus.FILLED, OrderStatus.PARTIALLY_FILLED):
                                try:
                                    is_stale = exec_engine.is_order_stale(internal_id, ttl_seconds)
                                except Exception:
                                    is_stale = True
                                if is_stale:
                                    exec_engine.cancel_order(
                                        internal_order_id=internal_id,
                                        broker_order_id=broker_order_id,
                                        reason="ttl_expired_no_chase",
                                    )
                                    journal.write_event({"event": "order_cancelled", "trade_id": trade_id, "internal_order_id": internal_id, "broker_order_id": broker_order_id, "reason": "ttl_expired_no_chase"})
                                    logger.info("LIMIT_TTL_EXPIRED_CANCELLED", extra={"symbol": sig_symbol, "strategy": sig_strategy, "ttl_seconds": ttl_seconds})
                                continue
                            ...
                            journal.write_event({"event": "order_submitted", "trade_id": trade_id, "internal_order_id": internal_id, "broker_order_id": broker_order_id, "symbol": sig_symbol, "side": side_str, "qty": str(qty), "order_type": "MARKET", "strategy": sig_strategy})
                        ...
                        if broker_side == BrokerOrderSide.BUY:
                            ...
                            if stop_price is not None and filled_qty is not None:
                                try:
                                    ...
                                    journal.write_event({"event": "protective_stop_submitted", "trade_id": trade_id, "symbol": sig_symbol, "stop_broker_order_id": stop_id, "stop_price": str(stop_price_dec), "qty": str(filled_qty)})
                        ...
                        if filled_qty is not None and fill_price is not None:
                            journal.write_event({"event": "order_filled", "trade_id": trade_id, "internal_order_id": internal_id, "broker_order_id": broker_order_id, "symbol": sig_symbol, "side": side_str, "filled_qty": str(filled_qty), "fill_price": str(fill_price)})
                ...
                if cycle_count >= orphan_check_interval:
                    ...
                    try:
                        ...
                        if orphans:
                            logger.error(f"ORPHAN ORDERS DETECTED: {len(orphans)} orders", extra={"orphan_broker_ids": orphans, "action": "Manual review required"})

                        shadows = order_tracker.get_shadow_orders(broker_orders)
                        if shadows:
                            logger.error(f"SHADOW ORDERS DETECTED: {len(shadows)} orders", extra={"shadow_client_ids": shadows, "action": "Manual review required"})
                ...
                # For tests/run_once, fail fast rather than looping/sleeping
                if opts.run_once:
                    return 1
        ...
        # Always stop container services, even on early safety halts.
        try:
            container.stop()
        except Exception:
            pass
