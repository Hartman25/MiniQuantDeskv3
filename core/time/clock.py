...
def local_time_str(fmt: str = "%Y-%m-%d %H:%M:%S %Z", dt: Optional[datetime] = None) -> str:
    """
    Return a string of local time using the host machine's timezone, with the correct
    timezone label (e.g., HST/PST/EST) based on the OS settings.

    - If dt is None: uses current local time.
    - If dt is provided: converts dt to local timezone before formatting.
    """
    if dt is None:
